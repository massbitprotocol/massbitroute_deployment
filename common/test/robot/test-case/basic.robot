...
*** Variables ***
${SCHEDULER_SERVER}  http://localhost:3031
${WORKER_SERVER}  http://localhost:4040
...
Deploy test-solana-block, then check if data was inserted into DB
    # Configuration
    Connect To Database  psycopg2  postgres  massbit-fisherman  postgres  172.24.10.13  5432
...
Pooling Status
    [Arguments]  ${payload}
    ${status_res} =    GET  ${SCHEDULER_SERVER}/compile/status/${payload}  expected_status=200
