...
cat $ENV_DIR/templates/gateway-docker-compose.yaml.template | \
  ...
  sed "s/\[\[ZONE\]\]/$zone/g" \
	> $ENV_DIR/gateways/docker-compose-${DOCKER_ID}.yaml
docker-compose -f $ENV_DIR/gateways/docker-compose-${DOCKER_ID}.yaml up -d --force-recreate
