...
for (( k = 0; k<$LOOP; k++ )); do
  ...
  cat $ENV_DIR/templates/node-docker-compose.yaml.template | \
      ...
      sed "s/\[\[ZONE\]\]/$zone/g" \
      > $ENV_DIR/nodes/docker-compose-${DOCKER_ID}.yaml
  #State3: Create docker node
  docker-compose -f $ENV_DIR/nodes/docker-compose-${DOCKER_ID}.yaml up -d --force-recreate
